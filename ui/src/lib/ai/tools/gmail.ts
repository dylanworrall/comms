...
export const gmailTools = {
  search_gmail: tool({
    ...
    execute: async ({ query, limit, account }) => {
      try {
  ...
  send_gmail: tool({
    ...
    execute: async ({ to, subject, body, cc, account }) => {
      try {
  ...
  trash_gmail: tool({
    ...
    execute: async ({ messageId, account }) => {
      try {
        const { gmail } = getGmailClient(account);
  ...
  mark_gmail_read: tool({
    ...
    execute: async ({ messageId, unread, account }) => {
      try {
        const { gmail } = getGmailClient(account);
        await gmail.users.messages.modify({
          userId: "me",
  ...
  reply_gmail: tool({
    ...
    execute: async ({ threadId, body, account }) => {
      try {
        const { gmail, account: acct } = getGmailClient(account);
