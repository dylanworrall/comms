...
async function validateAnthropicKey(key: string): Promise<boolean> {
  try {
    ...
    // 200 = valid, 401 = invalid, anything else (rate limit, etc) = assume valid
...
export async function resendSetupFlow(): Promise<void> {
  const existingKey = getEnvVar("RESEND_API_KEY");
  const existingEmail = getEnvVar("COMMS_FROM_EMAIL");

  if (existingKey && existingEmail) {
    const { reconfigure } = await inquirer.prompt([{
      type: "confirm",
      name: "reconfigure",
      message: `Resend already configured (key: ${maskKey(existingKey)}, from: ${existingEmail}). Reconfigure?`,
      default: false,
    }]);
  ...
  const answers = await inquirer.prompt([
    {
      ...
      default: existingKey,
      validate: (input: string) => {
        if (!input.trim()) return "Resend API key cannot be empty";
        if (!input.startsWith("re_")) return "Key should start with re_";
        return true;
      },
    },
    {
      type: "input",
      name: "fromEmail",
      message: "From email address (for sending):",
      default: existingEmail || "Comms Client <onboarding@resend.dev>",
      validate: (input: string) => input.trim() ? true : "Email cannot be empty",
    },
  ]);

  setEnvVar("RESEND_API_KEY", answers.resendKey.trim());
  setEnvVar("COMMS_FROM_EMAIL", answers.fromEmail.trim());
  log.success(`Resend configured! From: ${answers.fromEmail.trim()}`);
}
