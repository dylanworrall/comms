...
export async function loadConfig(cliConfigPath?: string): Promise<CommsConfig> {
  let config = {
    ...DEFAULT_CONFIG,
  ...
  if (process.env.COMMS_FROM_EMAIL) config.fromEmail = process.env.COMMS_FROM_EMAIL;
  if (process.env.ANTHROPIC_MODEL) config.anthropicModel = process.env.ANTHROPIC_MODEL;

  return config as unknown as CommsConfig;
}
